...
class HygrothermographAccessory {
  constructor(log, config) {
    ...
    this.pendingMQTTValues = new Map();
  ...
  setIlluminance(newValue, force = false) {
    ...
    if (newValue == null || this.illuminanceService == null) {
      return;
    }
  ...
  setMoisture(newValue, force = false) {
    if (newValue == null || this.moistureService == null) {
      return;
    }
  ...
  setFertility(newValue, force = false) {
    if (newValue == null || this.fertilityService == null) {
      return;
    }
    ...
    // TODO: Custom Fertility Accessory and Characteristic
    // This is a workaround with most suitable Characteristic's value type
  ...
  addFakeGatoHistoryEntry() {
    if (
      ...
      (this.type === AccessoryType.Hygrotermograph && this.humidity == null) ||
      (this.type === AccessoryType.MiFlora && this.moisture == null)
    ) {
      return;
    }
    const humidity =
      this.type === AccessoryType.MiFlora ? this.moisture : this.humidity;
  ...
  flushPendingMQTTValues() {
    if (this.pendingMQTTValues.size === 0) {
      return;
    }
    this.log.debug(
      `Flushing ${this.pendingMQTTValues.size} pending MQTT value(s).`
    );
    for (const [topic, value] of this.pendingMQTTValues) {
      this.mqttClient.publish(topic, String(value), {
        qos: this.config.mqtt.qos || 0,
        retain: this.config.mqtt.retain || false,
      });
    }
    this.pendingMQTTValues.clear();
  }

  publishValueToMQTT(topic, value) {
    if (this.mqttClient == null || topic == null || value == null) {
      return;
    }
    if (this.mqttClient.connected === false) {
      this.pendingMQTTValues.set(topic, value);
      return;
    }
    this.mqttClient.publish(topic, String(value), {
      qos: this.config.mqtt.qos || 0,
